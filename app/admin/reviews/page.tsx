...
import { Check, Trash2 } from "lucide-react"; // J'ai retiré 'X' car inutilisé
...
export default async function AdminReviewsPage() {
    ...
    return (
        <div className="space-y-8 animate-in fade-in">
            <header className="flex flex-col md:flex-row md:items-center justify-between border-b border-beige/20 pb-6 gap-4">
                <div>
                    ...
                    <table className="w-full text-left text-sm min-w-[900px]">
                        <thead className="bg-off-white border-b border-beige/20 text-charcoal/60 uppercase tracking-widest text-[10px]">
                            <tr>
                                ...
                                <th className="p-4 font-normal w-64">Commentaire</th>
                            ...
                            {reviews.length === 0 ? (
                                ...
                                reviews.map((review) => (
                                    <tr key={review.id} className={`hover:bg-off-white/50 transition-colors ${!review.isApproved ? 'bg-orange-50/30' : ''}`}>
                                        ...
                                        <td className="p-4 max-w-xs text-charcoal/70" title={review.comment}>
