...
export default function CheckoutClient() {
    ...
    const { calculateShippingFee, SHIPPING_ZONES } = require('@/lib/shipping');

    // Calculate shipping based on city and cart total
    const shippingCost = calculateShippingFee(formData.city, cartTotal);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // Basic validation
        if (!formData.fullName.trim() || !formData.email.trim() || !formData.phone.trim() || !formData.city.trim() || !formData.address.trim() || !formData.terms) {
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="space-y-2">
                                    ...
                                    >
                                        ...
                                        <optgroup label="Zone 1 (Express)">
                                            {SHIPPING_ZONES.ZONE_1.map((city: string) => (
                                                <option key={city} value={city}>{city}</option>
                                            ))}
                                        </optgroup>
                                        <optgroup label="Autres Villes">
                                            {MOROCCAN_CITIES.filter(c => !SHIPPING_ZONES.ZONE_1.includes(c)).map(city => (
                                                <option key={city} value={city}>{city}</option>
                                            ))}
                                        </optgroup>
                                    </select>
